...
\begin{enumerate}
  \item We formalize a hierarchical piecewise-rigid displacement approach for exploded-view administrative cartography, in which each feature undergoes rigid-body motion while displacement vectors are generated by a continuous hierarchical spatial model. This places the method between rigid transforms, continuous deformation, and topological rearrangements.
...
Calibration on NJ (manual visual reference $\alpha_l = 10{,}000$\,m) yields $\gamma_l = 1.136$. Applying the same $\gamma_l$ to PA geometry ($\widetilde{R}_{\text{local}} = 116{,}086$\,m, $\bar{n} = 449$) predicts $\alpha_l = 12{,}447$\,m --- substantially smaller than the heuristic value of $40{,}000$\,m used in initial PA runs, confirming that the heuristic over-expanded municipalities locally. Sensitivity analysis across a $\pm 15\%$ range around this value shows stable mean displacement (CV $< 0.02$) and consistent visual structure. The evidence supports $\gamma_l = 1.136$ as a practical transferable default for the tested administrative datasets, but additional independently judged calibration cases are needed before treating it as universal.

\subsubsection{Empirical Finding: Hierarchical Tightness Ratio}
\label{sec:ratio}

A secondary finding of the validation study is the dimensionless ratio $\widetilde{R}_{\text{local}}/\bar{w}$, which measures how many characteristic polygon widths fit within a typical regional radius --- a measure of \emph{hierarchical tightness}. Reproducible validation across eleven US states reveals that this ratio varies by administrative system type rather than taking a single universal value.
...
\noindent The NJ $\alpha_l$ match is essentially exact ($<0.1\%$ error), reflecting the calibration of $\gamma_l = 1.136$ on the reference layout. The $\alpha_r$ prediction remains subject to regional-legibility calibration: $\gamma_r = 3.0$ is a working default whose residual error ($+13.9\%$ on NJ) produces a useful starting layout but may benefit from additional visual calibration (see Section~\ref{sec:ratio} for broader discussion).
...
\caption{Exploded-view map of New Jersey municipalities ($n = 564$) using the calibrated displacement values and documented display offsets. North, Central, and South regions are separated by the regional displacement term while each municipality's internal geometry is preserved exactly. The display offsets are stored in \texttt{paper/outputs/tables/nj\_drag\_offsets.csv}.}
...
Parameters $\gamma_r$ and $\gamma_l$ estimated from the New Jersey dataset were applied directly without additional tuning. Applying the geometry-derived formulas to the PA geometry ($\bar{w} = 6{,}725$\,m, $n_r = 6$, $\widetilde{R}_{\text{local}} = 116{,}086$\,m, $\bar{n} = 449$) yields $\alpha_r = 20{,}174$\,m and $\alpha_l = 12{,}447$\,m. These values are consistent with the cross-state calibration table (Table~\ref{tab:crossstate}). A focused sensitivity analysis varying $\alpha_l$ by $\pm 15\%$ around the formula-derived value confirmed stable mean displacement (CV $< 0.02$) and consistent mean neighbor gap across all levels, supporting the formula-derived value as a reproducible starting point.
...
\caption{Pennsylvania municipalities ($n = 2{,}572$) after formula-derived displacement ($\alpha_r = 20{,}174$\,m, $\alpha_l = 12{,}447$\,m, $p = 1.25$) and documented region-level display offsets. The offsets are rigid translations recorded in \texttt{paper/outputs/tables/pa\_drag\_offsets.csv}; they improve figure composition without changing any municipality's shape, area, or within-region relative arrangement.}
...
\caption{Pennsylvania displacement vector field. Grey polygons show the original municipal coverage, coloured polygons show the formula-derived exploded layout, and arrows connect each municipality's original centroid to its exploded centroid. The figure visualizes how the regional and local displacement terms pull municipalities apart while preserving each polygon by rigid translation.}
...
For the New Jersey case study, where manual visual reference values are available, the geometry-derived formulas produce the following predictions under working defaults ($\gamma_r = 3.0$, $\gamma_l = 1.136$):
...
The $\alpha_l$ prediction is effectively exact for the NJ reference layout. The $\alpha_r$ prediction error of 13.8\% reflects residual uncertainty in $\gamma_r$, which has not yet been stabilized across a sufficient number of independent visual reference cases. In practice, an $\alpha_r$ error of this magnitude produces a useful starting layout that may require documented display offsets or additional calibration of regional spacing.

\subsection{Displacement Field Coherence}

The diagnostic displacement vector field (Section~\ref{sec:diagnostic}) confirmed three expected structural properties across both case studies: coherent outward radial orientation within each region, larger displacement magnitudes near regional peripheries, and suppressed displacement near regional centroids consistent with the $s_i$ weighting function. These properties hold in both the three-region NJ configuration and the six-region PA configuration, indicating that the vector field structure is robust to variation in regional count and geographic extent.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Extended Validation: Additional U.S. States, Canada, and Germany}
\label{sec:extended}
% ─────────────────────────────────────────────────────────────────────────────

Additional cases were selected for extended validation targeting structural properties that the New Jersey and Pennsylvania case studies do not fully exercise. Ohio and Michigan test dense municipal systems with different regional geometries; Kentucky, Illinois, North Dakota, North Carolina, Virginia, Texas, and Florida broaden the U.S.\ comparison; Canada and Germany test transfer beyond a single U.S.\ administrative context.
...
\subsection{Additional U.S. Validation States}
...
\caption{Texas county subdivisions after formula-derived displacement and documented display offsets. The layout demonstrates the larger-unit regime identified in Table~\ref{tab:crossstate}.}
\label{fig:texas}
\end{figure}
...
\caption{Florida county subdivisions after formula-derived displacement and documented display offsets. The compact four-region hierarchy produces a lower tightness ratio than the dense municipal states.}
\label{fig:florida}
\end{figure}
...
\caption{Exploded-view map of Canadian census subdivisions, provinces only ($\alpha_r = 22.2$\,km, $\alpha_l = 76.1$\,km, $p = 1.25$, $\gamma_r = 3$, $\gamma_l = 1.136$), with documented display offsets applied for figure composition.}
\label{fig:canada_prov}
\end{figure}

\subsection{Germany: Non-US Administrative Regions}

Germany provides a second non-US validation using GADM administrative geometries, grouped by first-level federal states and displayed at a lower administrative level. The result is visually more crowded than the Canadian and U.S.\ state examples because many small units are concentrated in central and western Germany, but the same workflow applies: formula-derived displacement creates the analytical layout, and documented region-level offsets are applied only for final composition.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{germany_dragged_layout.png}
\caption{Germany administrative regions after formula-derived displacement and documented display offsets. The case is best interpreted as a stress test and supplemental example because the administrative density remains high in several neighboring regions.}
\label{fig:germany}
\end{figure}
...
\caption{HHS national layout with 10 numbered regions after grouped displacement and documented display offsets. Region numbers match the official HHS region numbering; the offsets are stored in \texttt{paper/outputs/tables/hhs\_drag\_offsets.csv}.}
\label{fig:hhs_auto}
\end{figure}
...
\caption{Documented HHS display-offset step. Grey shapes show the pre-offset grouped layout, coloured shapes show the final display layout, and arrows show the region-level translations selected with the drag helper.}
...
\end{itemize}
All case studies are reproducible from public U.S.\ Census TIGER/Line, Statistics Canada, and GADM boundary data using the scripts and offset tables in \texttt{paper/scripts} and \texttt{paper/outputs/tables}. No proprietary data or software is required.

\subsection*{Interactive Map Gallery}

An accompanying interactive gallery is available at \url{https://prigasg.github.io/explodemap/map-gallery/}. The package repository is available at \url{https://github.com/PrigasG/explodemap}. The gallery corresponds to package version \texttt{0.2.0}; accessed May 26, 2026.
