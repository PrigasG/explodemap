...
\begin{enumerate}
  ...
  \item A generalized open implementation (\texttt{explode\_any.R}) applicable to any administrative polygon dataset without modification to the core algorithm (Section~\ref{sec:genimpl}), together with a companion focus-map widget for interactive selected-feature inspection.
\end{enumerate}
...
\begin{itemize}
  \item \texttt{explode\_any.R} --- generalized implementation with three entry points (\texttt{explode\_state}, \texttt{explode\_sf}, \texttt{explode\_sf\_with\_lookup}), applicable to any projected polygon dataset with any grouping.
  \item \texttt{explode\_map\_states.R} --- multi-state calibration runner covering nine US states with disk-based caching and cross-state $\gamma$ stability reporting.
  \item \texttt{explode\_map\_canada.R} --- non-US validation using Statistics Canada 2021 census subdivision boundaries.
  \item \texttt{hhs\_exploded\_map.R} --- three-level national grouped layout with collision-aware anchor refinement (Algorithm~2).
  \item \texttt{explode\_map\_app/app.R} --- interactive Shiny application for side-by-side visual validation and live calibration logging.
