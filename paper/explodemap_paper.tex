...
More broadly, existing cartographic transformations include rigid transforms (translation, rotation, scaling), continuous deformation (cartograms, rubber-sheet maps, diffusion-based warping), and topological rearrangements (schematic maps, grid maps). The present method formalizes a \emph{piecewise rigid hierarchical displacement} approach: each feature undergoes rigid-body motion, while the displacement vectors are sampled from centroid-derived radial potentials. This specific combination has not been systematically formalized for administrative exploded-view cartography in the literature reviewed here.
...
\]
Since $r_i < r_j$ and $p > 0$, it follows that $\delta_i < \delta_j$. Each unit is displaced along its own unit vector $\hat{d}_{\text{local}}$ away from $C_r$, so the displaced radial distance satisfies:
\[
...
\noindent The radial gap model determines the structural form of the regional displacement parameter. However, the minimum desired clearance between adjacent region boundaries is not determined purely by geometry; it also reflects cartographic design considerations such as legibility and whitespace requirements. The required gap is therefore expressed as $g_r = \gamma_r\,\bar{w}$, where $\bar{w}$ is the characteristic polygon diameter and $\gamma_r$ is a dimensionless clearance coefficient. Substituting into the gap criterion yields:
\begin{equation}
...
Calibration on NJ yields $\gamma_r = 2.64$ from a manual visual reference of $\alpha_r = 6{,}000$\,m. Earlier PA experiments imply a second, partially heuristic value near $\gamma_r = 3.72$. These values are directionally consistent but not sufficient to fix $\gamma_r$ as universal. The working default used here is therefore $\gamma_r = 3.0$, reported as an empirical legibility coefficient requiring additional independently judged calibration.
...
A secondary finding of the evaluation is the dimensionless ratio $\widetilde{R}_{\text{local}}/\bar{w}$, which measures how many characteristic polygon widths fit within a typical regional radius --- a measure of \emph{hierarchical tightness}. Reproducible scripts across eleven US states using county-subdivision boundaries (Table~\ref{tab:crossstate}) show that this ratio varies by administrative system type rather than taking a single universal value.

\begin{table}[h]
\centering
\caption{Cross-state geometry statistics and derived parameters ($\gamma_r = 3.0$, $\gamma_l = 1.136$). U.S.\ state entries use Census TIGER/Line \emph{county subdivision} (COUSUB) boundaries; Canada uses Statistics Canada CSD boundaries. Five additional county-level state layouts (TN, GA, MN, CA, CO) are included in the interactive gallery as supplemental visual demonstrations but are not part of this table (see Section~\ref{sec:supporting}). All values generated from \texttt{paper/scripts/02\_cross\_state\_validation.R}; runtime excludes first-time data download.}
...
Each U.S.\ state uses a UTM zone centred on its geographic extent; exact EPSG codes
are listed in the state registry (\texttt{paper/scripts/state\_registry\_paper.R}).}
\label{tab:crs}
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{llll}
\toprule
\textbf{Dataset} & \textbf{EPSG} & \textbf{CRS name} & \textbf{Reason} \\
\midrule
New Jersey       & 32111 & NAD83 / New Jersey (State Plane) & Local State Plane metric projection \\
Pennsylvania     & 26918 & NAD83 / UTM zone 18N & Local metric displacement and diagnostics \\
Ohio             & 32617 & WGS84 / UTM zone 17N & State-scale metric displacement \\
Michigan         & 32616 & WGS84 / UTM zone 16N & State-scale metric displacement \\
Kentucky         & 32616 & WGS84 / UTM zone 16N & State-scale metric displacement \\
Illinois         & 32616 & WGS84 / UTM zone 16N & State-scale metric displacement \\
North Dakota     & 32614 & WGS84 / UTM zone 14N & State-scale metric displacement \\
North Carolina   & 32617 & WGS84 / UTM zone 17N & State-scale metric displacement \\
Virginia         & 32618 & WGS84 / UTM zone 18N & State-scale metric displacement \\
Tennessee        & 32616 & WGS84 / UTM zone 16N & State-scale metric displacement \\
Georgia          & 32617 & WGS84 / UTM zone 17N & State-scale metric displacement \\
Minnesota        & 32615 & WGS84 / UTM zone 15N & State-scale metric displacement \\
California       & 32610 & WGS84 / UTM zone 10N & State-scale metric displacement \\
Colorado         & 32613 & WGS84 / UTM zone 13N & State-scale metric displacement \\
Texas            & 32614 & WGS84 / UTM zone 14N & Large-unit state geometry \\
Florida          & 3086  & NAD83 / Florida GDL Albers & Large-unit state geometry \\
HHS grouped layout & 5070 & NAD83 / Conus Albers & National U.S.\ metric consistency \\
Canada           & 3347  & NAD83 / Statistics Canada Lambert & National metric consistency across provinces \\
Germany          & 25832 & ETRS89 / UTM zone 32N & Metric displacement for non-U.S.\ administrative units \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\medskip
Table~\ref{tab:pa_sensitivity} summarises the $\pm 15\%$ sensitivity sweep for PA.
The formula-derived value $\alpha_l = 12{,}447$\,m is the centre column; the flanking
columns apply a uniform scalar of $0.85\times$ and $1.15\times$ respectively.
Quantities are computed from the original geography and the formula-derived exploded
layout without display offsets.

\begin{table}[H]
\centering
\caption{Pennsylvania $\alpha_l$ sensitivity: $\pm 15\%$ around the formula-derived
value ($\alpha_r = 20{,}174$\,m fixed, $p = 1.25$, $n = 2{,}572$).
``\% $<50$\,m'' is the percentage of adjacent unit pairs whose post-explosion
minimum boundary distance remains below 50\,m.}
\label{tab:pa_sensitivity}
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lrrr}
\toprule
\textbf{Metric} & $\bm{\alpha_l \times 0.85}$ \textbf{(10,580\,m)} &
                  $\bm{\alpha_l \times 1.00}$ \textbf{(12,447\,m)} &
                  $\bm{\alpha_l \times 1.15}$ \textbf{(14,314\,m)} \\
\midrule
Mean $\|t_i\|$ (m)      & $\approx 17{,}900$ & $\approx 20{,}630$ & $\approx 23{,}400$ \\
Mean neighbor gap (m)   & $\approx 55$       & $\approx 67$       & $\approx 80$       \\
Minimum gap (m)         & 0                  & 0                  & 0                  \\
\% pairs $<50$\,m       & $\approx 70\%$     & $\approx 65.7\%$   & $\approx 61\%$     \\
Residual contacts       & higher             & baseline           & lower              \\
CV (mean $\|t_i\|$)     & $< 0.02$           & $< 0.02$           & $< 0.02$           \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

\noindent The coefficient of variation of mean displacement remains below $0.02$
across the full $\pm 15\%$ range, confirming smooth parameter response. The minimum
gap stays at zero in all three columns because the dense Philadelphia metro core is
structurally under-expanded at all tested levels; this is a density-driven phenomenon,
not a centroid-proximity artefact. The formula-derived centre value is therefore a
reproducible analytical starting point, not an optimised spacing solution.
...
Additional cases were selected for extended evaluation targeting structural properties that the New Jersey and Pennsylvania case studies do not fully exercise. Ohio and Michigan test dense municipal systems with different regional geometries; Kentucky, Illinois, North Dakota, North Carolina, Virginia, Tennessee, Georgia, Minnesota, California, Colorado, Texas, and Florida broaden the U.S.\ comparison across dense municipal, large-unit, and regionally diverse geographies; Canada and Germany test reproducibility beyond a single U.S.\ administrative context.
...
Broad reproducibility tests & Illinois, North Dakota, North Carolina, Kentucky, Virginia, Tennessee, Georgia, Minnesota, California, Colorado, Texas, Florida & Parameter table, scripts, static outputs, documented offsets \\
...
Five additional U.S.\ states using county-subdivision boundaries were added to broaden the tightness-ratio comparison. Illinois ($n=1{,}694$, ratio 16.44), North Dakota ($n=1{,}761$, ratio 18.76), and North Carolina ($n=1{,}041$, ratio 15.27) fall in the higher tightness-ratio band with the dense municipal cases. Kentucky ($n=493$, ratio 8.46) and Virginia ($n=582$, ratio 10.07) fall closer to the larger-unit band. All five are included in Table~\ref{tab:crossstate}.

Five additional county-level state layouts --- Tennessee (95 counties, 3 regions), Georgia (159 counties, 5 k-means regions), Minnesota (87 counties, 4 k-means regions), California (58 counties, 5 k-means regions), and Colorado (64 counties, 3 regions) --- are included in the interactive gallery as supplemental visual demonstrations. Because these datasets use county-level units rather than county-subdivision units, they are not included in the tightness-ratio table and are not used for parameter calibration. Reproducible drag helpers and static layout outputs are present in \texttt{paper/outputs/drag\_helpers}.
...
The extended evaluation covering eleven US states plus Canada and Germany shows that the dimensionless ratio $\widetilde{R}_{\text{local}}/\bar{w}$ is a useful diagnostic of administrative tightness. Fine-grained township and county states including NJ (15.83), PA (17.26), IL (16.44), ND (18.76), NC (15.27), OH (12.92), and MI (17.81) occupy a higher ratio band than larger-unit cases such as KY (8.46), VA (10.07), TX (8.56), and FL (7.57). Canadian provinces occupy a qualitatively different regime at 111.23. Five additional county-level state layouts (Tennessee, Georgia, Minnesota, California, and Colorado) are included in the interactive gallery as supplemental visual demonstrations and are not part of the tightness-ratio calibration. Michigan's result is particularly useful: despite a geographic discontinuity between its Upper and Lower Peninsulas and a bimodal municipal size distribution, the median-based $\bar{w}$ remains well behaved.
...
% -----------------------------------------------------------------------
% SUBMISSION CHECKLIST — fill in the three tagged lines before uploading:
%   SUBMISSION-REF  : journal manuscript number, e.g. CAGIS-2026-0042
%   ZENODO-DOI      : 10.5281/zenodo.XXXXXXX  (mint after final commit)
%   FINAL-SHA       : 7-char SHA of the repository state used by the manuscript
% -----------------------------------------------------------------------
% SUBMISSION-REF: [to be assigned]
% ZENODO-DOI: 10.5281/zenodo.20402131
% FINAL-SHA: 1952569  (tag v0.2.0 = 5948bb7; update after final sync commit)

The reference implementation is available as an open-source, MIT-licensed
\textsf{R}~package (\textsf{R}~$\geq 4.1.0$ required).
The package, paper scripts, and interactive gallery used in this manuscript
correspond to \textbf{version 0.2.0}, tagged \texttt{v0.2.0} in the public
repository at:
...
\noindent A citable copy of the manuscript PDF is deposited at
Zenodo~\citep{zenodo_explodemap}. The DOI \texttt{10.5281/zenodo.20402131}
identifies the submitted paper artifact. The reproducible source code, paper
scripts, display-offset tables, and interactive gallery are maintained in the
public GitHub repository; readers wishing to reproduce results should use the
tagged release and the repository history linked from GitHub rather than relying
only on the moving live branch.

The core release includes:
\begin{itemize}
  \item \texttt{explode\_sf()}, \texttt{explode\_state()}, and
        \texttt{explode\_sf\_with\_lookup()} --- generalized entry points for
        projected polygon datasets with arbitrary grouping columns, lookup
        tables, or U.S.\ TIGER/Line inputs.
  \item \texttt{explode\_grouped()} --- three-level national and multi-region
        grouped layouts with collision-aware anchor refinement (Algorithm~2).
  \item \texttt{apply\_region\_offsets()} --- reproducible application of
        documented region-level display offsets stored as CSV files.
  \item Installed example scripts in \texttt{inst/examples/} ---
        calibration, Canada, HHS, lookup-table, manual-tuning,
        collision-refinement, and focus-map workflows, all runnable without
        proprietary data.
  \item Shiny focus-map examples --- interactive selected-feature
        inspection via \texttt{focusmapOutput()} and \texttt{renderFocusmap()}.
  \item \texttt{focus\_map()} --- companion htmlwidget for viewport focusing
        and information cards; its interaction model is developed separately
        from the displacement field described here.
\end{itemize}

\noindent All case studies reproduce from public U.S.\ Census TIGER/Line,
Statistics Canada, and GADM boundary data using the scripts and offset tables
in \texttt{paper/scripts} and \texttt{paper/outputs/tables}.
No proprietary data or software is required.
Package checks run with \texttt{R CMD check}; the manuscript rebuilds from
\texttt{paper/explodemap\_paper.tex} with \texttt{pdflatex} after regenerating
figures via \texttt{paper/scripts}.

\subsection*{Interactive Map Gallery}

An interactive gallery of all case studies is available at:
\begin{quote}
\url{https://prigasg.github.io/explodemap/map-gallery/index.html}
\end{quote}
The gallery corresponds to package version \texttt{0.2.0} and was accessed
\mbox{26 May 2026}. For each case it lists the formula-derived parameters,
whether documented display offsets were applied, and links to the
corresponding offset CSV files. Display-offset layouts are used for
publication figures and web presentation; validation metrics are computed
from the original geography and the formula-derived layouts unless
explicitly stated otherwise. The output taxonomy is defined in
Table~\ref{tab:output-types}.
...
\bibitem[Arthur(2026a)]{arthur2026paper}
Arthur, G. (2026).
A hierarchical vector-based framework for multi-scale exploded-view
cartography: centroid-driven spatial displacement for dense
administrative maps.
% SUBMISSION-REF: [journal + manuscript number to be added on acceptance]
\textit{Working paper}.

\bibitem[Arthur(2026b)]{zenodo_explodemap}
Arthur, G. (2026).
\textit{explodemap: Hierarchical Exploded-View Cartography} (Version 0.2.0)
[Software].
Zenodo.
% ZENODO-DOI: 10.5281/zenodo.20402131
\url{https://doi.org/10.5281/zenodo.20402131}

% ---  cartographic and methodological references  --------------------------
...
A vector field model to handle the displacement of multiple conflicts in
building generalization.
\textit{International Journal of Geographical Information Science}, 29(8),
1310--1331.
\url{https://doi.org/10.1080/13658816.2015.1019886}

\bibitem[Bader(2001)]{bader2001}
Bader, M. (2001).
\textit{Energy minimization methods for feature displacement in map
generalization}.
...
Cecconi, A. (2003).
\textit{Integration of cartographic generalization and multi-scale databases
for enhanced web mapping}.
...
Studying cartographic heritage: Analysis and visualization of geometric
distortions.
\textit{Computers \& Graphics}, 35(2), 170--178.

\bibitem[Mackaness et~al.(2011)]{mackaness2011}
Mackaness, W.~A., Ruas, A., \& Sarjakoski, L.~T. (2011).
\textit{Generalisation of Geographic Information: Cartographic Modelling and
Applications}.
Elsevier.
...
Sun, S. (2020).
Applying forces to generate cartograms: a fast and flexible transformation
framework.
...
In \textit{Advances in Cartography and Geographic Information Science}.
Springer.

\bibitem[Yan \& Wang(2019)]{yan2019}
Yan, H., \& Wang, J.~Y. (2019).
\textit{Description Approaches and Automated Generalization Algorithms for
Groups of Map Objects}.
Springer.
