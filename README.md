...
`d_edit()` renders the widget; it does not modify `state` in place. In Shiny,
capture the user's edits back into R with `d_widget_state()` (the widget
reports to `input$<outputId>_state`):
